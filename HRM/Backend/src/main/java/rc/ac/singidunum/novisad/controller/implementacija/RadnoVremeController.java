...
import rc.ac.singidunum.novisad.service.implementacija.RadnoVremeService;
import rc.ac.singidunum.novisad.service.implementacija.ZaposleniService;

@Controller
@RequestMapping("/api/radnoVreme")
public class RadnoVremeController extends GenericCrudController<RadnoVremeDTO, RadnoVreme, Long> {
    ...
    @Override
    protected CrudService<RadnoVremeDTO, RadnoVreme, Long> getService() {
        return rvService;
    }
    
    @RequestMapping("/me")
    public ResponseEntity<List<RadnoVremeDTO>> getRadnoVremeZaUlogovanog(Authentication auth) {
        try {
            List<RadnoVremeDTO> radnaVremena = rvService.getRadnoVremeZaZaposlenog(auth.getName());
